...
export default function MapView() {
  ...
  return (
    <div style={{ height: 'calc(100vh - 65px)', width: '100%', position: 'relative' }}>
      {loading && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[1000] bg-white px-4 py-2 rounded-full shadow text-sm">
          Loading complaints...
        </div>
      )}

      {/* Legend */}
      <div className="absolute bottom-8 right-4 z-[1000] bg-white rounded-xl shadow-lg p-4 flex flex-col gap-2">
        <p className="text-xs font-semibold text-neutral-700 mb-1">Status</p>
        {[
      ...
      {/* Complaint count */}
      <div className="absolute top-4 left-4 z-[1000] bg-white rounded-xl shadow px-4 py-2">
            ...
            >
              <Popup>
                <div className="flex flex-col gap-1 min-w-[180px]">
                  ...
                  <p className="text-xs text-neutral-500">{c.category} · {c.priority}</p>
                  <p className="text-xs text-neutral-500">{c.addressText}</p>
